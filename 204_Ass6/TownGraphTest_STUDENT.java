...
public class TownGraphTest_STUDENT {
	...
	public void testAddEdge() {
		...
		graph.addEdge(town[3], town[10], 1, "Road_13");
		assertEquals(true, graph.containsEdge(town[3], town[10]));
	}
	...
	public void testTown_1ToTown4() {
		...
		if (beginIndex != null && endIndex != null) {
			...
			ArrayList<String> path = graph.shortestPathSets(beginIndex, endIndex);
			assertNotNull(path);
			assertTrue(path.size() > 0);
			assertEquals("Town_1 via Road_2 to Town_3 4 mi", path.get(0).trim());
			assertEquals("Town_3 via Road_13 to Town_4 2 mi", path.get(1).trim());
		} else
	...
	public void testTown_4ToTown_11() {
		...
		if (beginIndex != null && endIndex != null) {
			...
			assertNotNull(path);
			assertTrue(path.size() > 0);
			assertEquals("Town_3 via Road_4 to Town_7 1 mi", path.get(0).trim());
			assertEquals("Town_5 via Road_10 to Town_10 5 mi", path.get(1).trim());
			assertEquals("Town_6 via Road_7 to Town_9 3 mi", path.get(2).trim());
		} else
			fail("Town names are not valid");
